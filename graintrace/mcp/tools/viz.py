...
def visualize(
    ...
    """
    # Lazy: pyvista/VTK is heavy and optional; import only when rendering.
    from graintrace.mcp import render  # pylint: disable=import-outside-toplevel
...
def plot_centroids(
    ...
    out_png : output PNG path (defaults under the MCP workdir /plots).
    """
    # Lazy + Agg-before-pyplot ordering: heavy plotting deps, imported on use.
    ...
    for ax, (a, b) in zip(axes, pairs):
        sc = ax.scatter(
            ...
            c=cvals,
            s=point_size,
            cmap="tab10" if csv_paths else "viridis",
            alpha=0.8,
        )
        ax.set_xlabel(a)
        ax.set_ylabel(b)
    ...
    fig.suptitle(
        title or (f"Centroids ({len(df)} grains)" + (f" by {clabel}" if clabel else ""))
    )
