...
_SKIP_DIRS = {
    ".git", "__pycache__", ".pytest_cache", "node_modules", "external", "dist",
    "build", ".vscode", "graintrace_mcp_out", "graintrace.egg-info",
    "out", "_truth",  # demo outputs
}
_MAX_BYTES = 400_000
...
@mcp.tool()
def list_code_files(subdir: Optional[str] = None, pattern: str = "*.py",
    ...
    for base in bases:
        ...
        for p in _iter_files(base, pattern):
            ...
            if len(out) >= max_files:
                break
    return {"repo_root": str(root), "n_files": len(out),
            "truncated": len(out) >= max_files, "files": sorted(out)}


@mcp.tool()
def read_code_file(path: str, start_line: Optional[int] = None,
    ...
    if p.stat().st_size > _MAX_BYTES and not (start_line or end_line):
        return {"error": f"file too large ({p.stat().st_size} bytes); pass a line range"}
    try:
...
@mcp.tool()
def search_code(query: str, subdir: Optional[str] = None, pattern: str = "*.py",
                regex: bool = False, max_results: int = 150) -> dict:
    ...
    """
    root = repo_root()
    bases = [str(_resolve(subdir))] if subdir else [
        str(root / d) for d in _CODE_ROOTS if (root / d).exists()]
    ...
    for ln in lines[:max_results]:
        try:
            ...
            rel.append(ln)
    return {"query": query, "n_matches": len(lines),
            "truncated": len(lines) > max_results, "matches": rel[:max_results]}
