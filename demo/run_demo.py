...
SCULPT_CONFIG = _tp.sculpt_config() or {
    ...
    "nprocs": 8,
    "environment": {"OPAL_LIBDIR": f"{_CUBIT_BIN}/mpi/lib",
...
CPFE = dict(device="cuda:0", ncore=16, device_batch=20000,
            dt=0.05, initialize_time=0.5,   # dtmax=10*dt=0.5 s; total_time derived
            grid_elems=[16, 16, 16], element_order="FIRST")  # SCULPT -> hex8
...
BASE_MATERIAL = dict(
    slip_constant_strength=100.0, voce_hardening_initial_slope=1650.0,
    voce_hardening_saturation=220.0, power_slip_n=25, power_slip_g0=1e-4,
    elastic_E=109000.0, elastic_nu=0.307, elastic_G=41700.0, burger_scale=2.54,
)
# calibrated opt-var name -> CPFE material name
CALIB_MAP = {
    "elastic_tensor_E": "elastic_E", "elastic_tensor_G": "elastic_G",
...
def stitch(meta):
    ...
    stitcher = RegionBaseStitching(
        scan_files=scans, output_csv=str(out_csv),
        position_tolerance=50, orientation_tolerance=5.0, radius_tolerance=-1,
        weights={"pos": 0.1, "ori": 1.0, "rad": 0}, min_neighbors=5,
        orientation_convention=meta["sample"]["units"]["orientation_convention"],
        orientation_units="degrees", symmetry=meta["sample"]["units"]["symmetry"],
        output_column=cols,
    )
...
def calibrate(meta, stitched_csv):
    ...
    M = oh.euler_to_matrix(df[["Eul0", "Eul1", "Eul2"]].to_numpy(),
                           "bunge", "degrees").numpy().reshape(len(df), 9)
    ...
    calib = MaterialCalibration(
        ...
        model_args=dict(neml2_path=cpfe_base + "/neml2_cpfe_calibration.i",
                        npoints=30, nchunk=2, device="cuda", compile=False,
                        assumed_rate=ASSUMED_RATE),  # == CPFE rate (consistency)
        data_args=dict(data_dir=str(calib_data),
                       strain_stress_file=str(EXP / "strain-stress.csv"),
                       npoints=30, full_field_strain_units="microstrain",
                       straintype="eKen", max_strain=0.006,
                       n_grains=min(100, len(df)), seed=42),
        save_dir=str(save_dir), apply_elastic_correction=False,
    )
...
def _material_from_json(path):
    ...
    print("    calibrated material:", {k: round(mat[k], 4) for k in
          ("elastic_E", "elastic_nu", "elastic_G", "slip_constant_strength",
           "voce_hardening_initial_slope", "voce_hardening_saturation")})
    return mat
...
def reconstruct(meta, stitched_csv):
    ...
    builder = VoronoiMeshBuilder(
        input_csv=str(stitched_csv), output_dir=str(out_ff), bounding_box=bbox, dim=3,
        auto_fix_bbox=True, bbox_fix_mode="remove_points", bbox_tolerance=2.5,
        angle_identifier=["Eul0", "Eul1", "Eul2"],
        orientation_descriptor="euler-bunge", orientation_active_convention=True,
        unit=unit, elastic_strain_identifier=EKEN_COLS, strain_unit="microstrain",
    )
    builder.build_voronoi(generate_mesh=False, option="centroid", CVT_iter=CVT_ITER,
                          morphoalgo="subplex", mesh_quality_min=0.7,
                          relative_el_size=REL_EL_SIZE, tesr_size=TESR_SIZE)
    vox = VoxelMeshBuilder(
        file_path=str(out_ff / "reconstruction_reformatted.csv"),
        save_dir=str(mesh_dir), euler_cols=["Eul0", "Eul1", "Eul2"],
        cell_id_col="CellID", angle_convention="bunge", angle_type="degrees",
        symmetry="432",
    )
    merged = vox.reconstruct(apply_smoothing=False)
    mesh_path = vox.mesh(sculpt_config=SCULPT_CONFIG,
                         sculpt_options=list(SCULPT_OPTIONS), merged_grid=merged)
...
def run_cpfe(meta, out_ff, material):
    ...
    print(f"    rate {strain_rate:.2e}/s (== calibration {ASSUMED_RATE:.0e}) -> "
          f"loading {loading_time:.6g}s, total_time {total_time:.6g}s")

    sim = CPFESimulation(
        mesh_file=str(mesh_e),
        save_simulation_folder=str(out_sim), moose_run_file=MOOSE_RUN_FILE,
        element_order=CPFE["element_order"],
        eeres_file=str(out_ff / "reconstruction_cpfe_ee.csv"),
        ori_file=str(ori_csv), dim=3, use_ff_initial_field=True,
    )
    sim.set_parameters("material", **material)
    sim.set_parameters("simulation_parameters", dt=CPFE["dt"],
                       total_time=total_time, initialize_time=init_t,
                       device=CPFE["device"], device_batch=CPFE["device_batch"],
    ...
    sim.set_parameters("grid_properties", number_of_elements=CPFE["grid_elems"],
                       bounding_box=grid_bb)
    print(f"    launching CPFE (device={CPFE['device']}, ncore={CPFE['ncore']}, "
...
def _wait_for_cpfe(out_sim, total_time, timeout=CPFE_TIMEOUT_S):
    ...
    while time.time() - t0 < timeout:
        ...
        if re.search(r"terminate called|Segmentation fault|MPI_ABORT|"
                     r"Fatal error in|Command not found", logtxt):
            raise RuntimeError(
...
def rei(last_grid_csv):
    ...
    rare_criteria = RareCriteria(selector=lambda df: rcs.select_highest_scalar(
        df, k=REI["k"], required_cols=scalar_col, min_size=1))
    weight_cfg = WeightConfig(mode="rbf", power=2.0, sigma=None,
                              sigma_auto={"sample_size": 500_000, "random_state": REI["seed"],
                                          "quantile": 0.5})
    irc = IdentifyRareClusters(input_csv_path=str(last_grid_csv), id_col="id",
                               coord_cols=("x", "y", "z"))
    ...
    bundle = irc.run_clustering(
        ...
        gsc_run_kwargs=dict(spec=spec, graph_mode="grid",
                            manhattan_radius=REI["manhattan_radius"], grid_tol=1e-6,
                            n_jobs=REI["n_jobs"], weight_chunk_size=500_000,
                            segmenter="leiden", seed=REI["seed"], weight_cfg=weight_cfg,
        ...
        indicator_run_kwargs=dict(method_type="scipy_hierarchical", spec=spec_reduced,
                                  threshold=REI["threshold"], method="average",
                                  criterion="distance",
    ...
    out = irc.run_get_rare_cluster(
        ...
        output_vtk_path=base + "_rare_clusters.vtk", export_control="auto",
        background_block_id=1, first_rare_block_id=2, also_write_final_label=True,
        rare_reduced_stats_csv_path=stats_csv, use_sample_std=False)
...
def make_plots(out_ff, last_grid_csv, rei_out):
    ...
    try:
        ...
        if recon_vtk.exists():
            ipf = IPFProcessor(crystal_symmetry="432", sample_symmetry="432",
                               save_dir=str(plots))
            # output_file is resolved relative to save_dir -> pass a bare filename.
            rgb_vtk = ipf.add_block_rgb_to_vtk(
                vtk_file=str(recon_vtk), output_file="reconstruction_ipf.vtk",
                direction=[0.0, 0.0, 1.0], angle_convention="bunge",
                angle_type="degrees", orientation_fields=("Eul0", "Eul1", "Eul2"))
            render.render_vtk(str(rgb_vtk), str(plots / "grains_reconstruction.png"),
    ...
    # REI hotspots
    try:
        rare_vtk = rei_out[0].get("output_vtk_path") if isinstance(rei_out[0], dict) else None
        rare_vtk = rare_vtk or (str(OUT / "rei" / "rei_rare_clusters.vtk"))
        if Path(rare_vtk).exists():
            render.render_vtk(rare_vtk, str(plots / "rei_hotspots.png"),
                              threshold_rare=True, title="Rare clusters (nye-tensor)")
    except Exception as e:
    ...
    try:
        ...
        if block.exists():
            res = SimulationResults(
                block_csv=str(block), field_dir=str(fdir),
                field_naming=FieldFileNaming(prefix="out_element_centroid",
                                             index_width=4, sep="_", suffix=".csv"))
...
def _plot_grid_field(csv, out_png):
    ...
    ax[0].set_title("Nye-tensor norm distribution"); ax[0].set_xlabel("|nye|")
    sc = ax[1].scatter(df["x"], df["y"], c=df["nye_norm"], s=4, cmap="viridis")
    ax[1].set_title("Nye norm (x-y)"); ax[1].set_aspect("equal")
...
def report(stats_csv, out_ff):
    ...
    sev_col = next((c for c in stats.columns if c.endswith("nye_tensor_norm_mean_mean")),
                   None)
    ...
    try:
        if ori.exists():
            g = pd.read_csv(ori)
            gcoord = [c for c in (("x", "y", "z"), ("X", "Y", "Z")) if set(c) <= set(g.columns)]
            if gcoord:
                from scipy.spatial import cKDTree
                cc = list(gcoord[0])
                idcol = next((c for c in ("cell_id", "id", "grain_id", "GrainID")
    ...
    for _, r in order.iterrows():
        ...
        line = (f"  cluster {int(r.get('cluster_label', -1))} "
    ...
    print(f"plots: {OUT/'plots'} | rare VTK: {OUT/'rei'} | "
...
def main():
    ...
    _step("1/6 stitch scans");        stitched = stitch(meta)
    _step("2/6 material calibration"); material = calibrate(meta, stitched)
    _step("3/6 FF reconstruct + mesh"); out_ff = reconstruct(meta, stitched)
    _step("4/6 CPFE (run to completion)"); last_grid = run_cpfe(meta, out_ff, material)
    _step("5/6 rare-event identification"); rei_out = rei(last_grid)
    _step("6/6 plots"); make_plots(out_ff, last_grid, rei_out)
