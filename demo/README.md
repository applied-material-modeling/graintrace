...
(`deploy/tools.json`, or `~/.config/graintrace/tools.json`, or
`$GRAINTRACE_TOOLS_JSON`) — copy `deploy/tools.example.json` and fill in your
paths. Meshing uses **CUBIT/SCULPT hex** (the recommended path); GMSH tets are an
FF-only last resort.
...
python demo/generate_experiment.py     # once: builds experiment/ (~200 grains, NEPER)
python demo/run_demo.py                # the full pipeline, CPFE runs to completion
```

Outputs land in `demo/out/` (gitignored):
...
```bash
DEMO_REUSE="stitch,calibrate,reconstruct" python demo/run_demo.py   # re-run only CPFE->REI
```
...
## Run it live (Open WebUI / MCP)

See `CHAT_PROMPTS.md` for the chat sequence that reproduces this through the MCP
tools (each step previews, you confirm; heavy steps return a job id you poll).
